...
from ZhihuColumnRedis.items import ZhihucolumnItem

empty_word = 'null'


class ZhColumnSpider(RedisSpider):
    """Spider that reads urls from redis queue (myspider:start_urls)."""
    name = 'zh_column_redis'
    redis_key = 'zh_column_redis:start_urls'
    ...
    def parse_init_page(self, response):
        ...
        par = re.compile(
            '<img class="Avatar Avatar--large" width="60" height="60" src="(.*?)" srcSet="(.*?) 2x" alt="(.*?)"/>'
        ...
        try:
            if result['paging']['is_end'] == 'false' or 'False':
                for res in result['data']:
                    ...
                    item['articles_count'] = res['object']['articles_count'] if res['object'][
                        'articles_count'] else empty_word
                    ...
                    item['creator_name'] = res['object']['author']['name'] if res['object']['author'][
                        'name'] else empty_word
                    item['creator_url'] = res['object']['author']['url_token'] if res['object']['author'][
                        'url_token'] else empty_word
                    yield item
