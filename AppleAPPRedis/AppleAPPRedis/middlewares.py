...
from my_tools.ip_test import get_ip, test_ip, del_ip
...
class RandIPMiddleWare(object):
    ...
    def process_request(self, request, spider):
        ...
        ip_ls = [self.connect.lindex(proxy_key, i).decode('utf-8') for i in range(self.connect.llen(proxy_key))]
        print(len(ip_ls))
        if len(ip_ls) < 3:
            get_ip()
...
class AppleappDownloaderMiddleware(object):
    # Not all methods need to be defined. If a method is not defined,
