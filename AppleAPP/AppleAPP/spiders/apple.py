...
from AppleAPP.items import AppleappItem
...
class AppleSpider(scrapy.Spider):
    ...
    def parse(self, response):
        for x in range(connect.llen(keyword_key)):
            ...
            item = AppleappItem()
            item['kw'] = kw
            new_url = 'https://itunes.apple.com/search?term={}&country=CN&media=software'.format(kw)
            yield scrapy.Request(url=new_url, callback='', meta={'item': item})
