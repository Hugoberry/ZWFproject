...
class RandMiddleware(object):
    ...
    def process_request(self, request, spider):
        ...
        # rua = connect.get(str(random.randint(0, 30))).decode('utf-8')
    ...
    def process_response(self, request, response, spider):
        ...
        if "qcaptcha" in response.url:
            # kw = request.meta['item']["search_kw"]
            # connect.sadd('fail_key', kw)
            # # 302 的 url 需要重新抽取更换
            from_url = re.compile(r"com%2Fns%3Fq%3D(.*)%26src%3Dsrp%26tn%3Dnews&tk=").findall(response.url)[0]
            from_url = 'https://news.so.com/ns?q=%s&src=srp&tn=news' % unquote(from_url)
...
class Search360SpiderMiddleware(object):
...
class Search360DownloaderMiddleware(object):
    # Not all methods need to be defined. If a method is not defined,
