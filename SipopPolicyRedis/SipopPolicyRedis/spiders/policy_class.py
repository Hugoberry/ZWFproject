...
from SipopPolicyRedis.items import SipoppolicyItem
...
class PolicySpider(RedisSpider):
    ...
    name = 'policy_class'
    redis_key = 'policy_class:start_urls'
    allowed_domains = ['sipop.cn']
    start_urls = ['http://www.sipop.cn/module/gate/policy/list.html']

    def parse(self, response):
        # http://www.sipop.cn/patent-interface-web/policy/queryListPolicyForPage?appKey=
        # fichinfoPotal&accessToken=POLICY_TOKEN&pageNum=1&pageSize=10&provinceCode=&industryCode
        # =&cityCode=&technicalField=&region=&implementMode=&recipients=&highFrequencyWords=&orderDesc=desc&_=1543991842779
        first_url = 'http://www.sipop.cn/patent-interface-web/policy/queryListPolicyForPage?appKey=' \
                    'fichinfoPotal&accessToken=POLICY_TOKEN&pageNum=1&pageSize=10&provinceCode=&industryCode' \
                    '=&cityCode=&technicalField=&region=&implementMode=&recipients=&highFrequencyWords=&' \
                    'orderDesc=desc&_={}'.format(str(int(time.time() * 1000)))
    ...
    def parse_pages(self, response):
        ...
        for page in range(pages):
            list_url = 'http://www.sipop.cn/patent-interface-web/policy/queryListPolicyForPage?appKey=' \
                       'fichinfoPotal&accessToken=POLICY_TOKEN&pageNum={}&pageSize=10&provinceCode=&industryCode' \
                       '=&cityCode=&technicalField=&region=&implementMode=&recipients=&highFrequencyWords=&' \
                       'orderDesc=desc&_={}'.format(str(int(page + 1)), str(int(time.time() * 1000)))
    ...
    def parse_list(self, response):
        ...
        for policy_id_mode in policy_id_ls:
            ...
            # http://www.sipop.cn/patent-interface-web/policy/getPolicyWithPolicyId?appKey=fichinfoPotal&accessToken=
            # POLICY_TOKEN&policyId=2daf211d-082d-4138-b236-4c72438df7aa&_=1543992233866
