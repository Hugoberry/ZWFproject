...
class PolicySpider(RedisSpider):
    ...
    name = 'policy_read'
    redis_key = 'policy_read:start_urls'
    allowed_domains = ['sipop.cn']
    start_urls = ['http://www.sipop.cn/module/gate/policy/policiesInterpretationList.html']

    def parse(self, response):
        # http://www.sipop.cn/patent-interface-web/unscramble/ajax/ListUnscramble?appKey=
        # fichinfoPotal&accessToken=POLICY_TOKEN&pageNum=1&pageSize=10&contentAttributeCode=10&_=1544059017330
        first_url = 'http://www.sipop.cn/patent-interface-web/unscramble/ajax/ListUnscramble?appKey=' \
                    'fichinfoPotal&accessToken=POLICY_TOKEN&pageNum=1&pageSize=10&contentAttributeCode=10&_' \
    ...
    def parse_pages(self, response):
        ...
        for page in range(pages):
            list_url = 'http://www.sipop.cn/patent-interface-web/unscramble/ajax/ListUnscramble?appKey=' \
                       'fichinfoPotal&accessToken=POLICY_TOKEN&pageNum={}&pageSize=10&contentAttributeCode' \
                       '=10&_={}'.format(str(int(page + 1)), str(int(time.time() * 1000)))
    ...
    def parse_list(self, response):
        ...
        for policy_id_mode in policy_id_ls:
            ...
            # http://www.sipop.cn/patent-interface-web/policy/getPolicyWithPolicyId?appKey=
            # fichinfoPotal&accessToken=POLICY_TOKEN&policyId=88065ac7-09ef-4daf-8e57-85c925bf1179&_=1544057791321
