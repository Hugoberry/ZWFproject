...
connect = redis.Redis(host='127.0.0.1', port=6379, db=13)
...
class RandMiddleware(object):
    ...
    def process_request(self, request, spider):
        ...
        request.headers.setdefault('Host', 'panda.www.net.cn')
    ...
    def process_response(self, request, response, spider):
        ...
        if response.status == 200:
            return_code = re.compile('<returncode>(.*)</returncode>').findall(response.text)[0]
            if return_code != '200':
...
class DomaintestSpiderMiddleware(object):
...
class DomaintestDownloaderMiddleware(object):
    # Not all methods need to be defined. If a method is not defined,
